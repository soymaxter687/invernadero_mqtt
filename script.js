...
clienteMQTT.on('message', (tema, mensaje) => {
    ...
    if (tema === 'invernadero/ph') {
        ...
        if (payload < 6 || payload > 7) {
            phSpan.classList.remove('ph');
            phSpan.classList.add('parpadeo', 'valor'); // Agregar la clase de parpadeo al texto
            ...
            phSpan.classList.remove('parpadeo', 'valor'); // Remover la clase de parpadeo del texto
            phSpan.classList.add('ph');
...
function alternarBoton() {
    ...
    const estado_agua = document.getElementById('icono-agua');

    if (boton.innerText === "Regar plantas") {
        boton.innerText = "Detener riego";
        boton.classList.remove('control-button-abierto');
        boton.classList.add('control-button-cerrado');
        estado_agua.src = "https://i.ibb.co/5hZFGRC/agua-cerrada.png"; // Imagen para estado cerrado
    } else {
        boton.innerText = "Regar plantas";
        boton.classList.remove('control-button-cerrado');
        boton.classList.add('control-button-abierto');
        estado_agua.src = "https://i.ibb.co/Qfmg8zW/agua-abierta.png"; // Imagen para estado abierto
    }
    
    // Remover el estado 'hover' en dispositivos táctiles
    boton.blur();
}

// Agregar evento para quitar el efecto hover en dispositivos táctiles
const boton = document.getElementById('boton-agua');
boton.addEventListener('touchend', () => {
    boton.classList.remove('control-button-abierto', 'control-button-cerrado');
});
